...
class ServerProcessManager extends EventEmitter {
  private servers: Map<number, ManagedServer> = new Map();
  private lineBuffers: Map<number, { stdout: string; stderr: string }> = new Map();
  private consoleHistory: Map<number, Array<ConsoleLogEntry>> = new Map();

  private getLineBuffer(serverId: number) {
  ...
  private addToHistory(serverId: number, entry: ConsoleLogEntry) {
  ...
  getConsoleHistory(serverId: number): Array<ConsoleLogEntry> {
    return this.consoleHistory.get(serverId) ?? [];
  }

  getStatus(serverId: number): { status: ServerStatus; pid: number | null; uptime: number | null } {
  ...
  async start(config: ServerConfig): Promise<{ success: boolean; error?: string }> {
    ...
    if (config.jvm_flags) {
      const customFlags = config.jvm_flags.split(' ').filter((f) => f.trim());
      jvmArgs.push(...customFlags);
    }

    jvmArgs.push('-jar', config.jar_file, 'nogui');
    ...
    try {
      ...
      proc.stdout?.on('data', (data: Buffer) => {
        ...
        lines.forEach((line) => {
          if (line.trim()) {
            ...
            this.addToHistory(config.id, { type: 'stdout', data: line, timestamp });
            this.emit('console:output', { serverId: config.id, type: 'stdout', data: line });
          }
        });
      });
      ...
      proc.stderr?.on('data', (data: Buffer) => {
        ...
        lines.forEach((line) => {
          if (line.trim()) {
            const timestamp = Date.now();
            this.addToHistory(config.id, { type: 'stderr', data: line, timestamp });
            this.emit('console:output', { serverId: config.id, type: 'stderr', data: line });
          }
        });
      });
  ...
  async stop(serverId: number): Promise<{ success: boolean; error?: string }> {
    const server = this.servers.get(serverId);
  ...
  getRunningServers(): Array<number> {
