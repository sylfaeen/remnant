...
import { scheduledTasks, taskExecutions, type ScheduledTask, type TaskConfig } from '@remnant/backend/db/schema';
...
class TaskScheduler {
  ...
  private async executeRestartTask(serverId: number, config: TaskConfig) {
  ...
  private async executeBackupTask(serverId: number, config: TaskConfig) {
  ...
  private async executeCommandTask(serverId: number, config: TaskConfig) {
