...
export class PaperMCService {
  ...
  async getBuilds(version: string): Promise<Array<number>> {
    const data = await graphql<{
  ...
  async getLatestBuild(version: string): Promise<{ number: number; downloadUrl: string; filename: string }> {
    const data = await graphql<{
      project: { version: { builds: { edges: Array<GraphQLBuildEdge> } } };
    }>(
      `
  ...
  async getBuildDownload(version: string, build: number): Promise<{ downloadUrl: string; filename: string }> {
    const data = await graphql<{
      project: { version: { builds: { edges: Array<GraphQLBuildEdge> } } };
    }>(
      `
        query ($version: String!, $after: String) {
          project(key: "paper") {
            version(key: $version) {
  ...
  async downloadJar(
    ...
    const { downloadUrl, filename } =
      build === 0 ? await this.getLatestBuild(version) : await this.getBuildDownload(version, build);
