...
import { sftpAccounts, servers } from '@remnant/backend/db/schema';
...
function runSftpScript(args: Array<string>): Promise<CommandResult> {
  return new Promise((resolvePromise) => {
    execFile('sudo', [SCRIPT_PATH, ...args], { shell: false, timeout: 15000 }, (error, stdout, stderr) => {
      if (error) {
        let errorMessage = `SFTP script failed: ${error.message}`;
        try {
...
export class SftpService {
  ...
  async listAccounts(serverId: number): Promise<Array<SftpAccountResponse>> {
  ...
  async cleanupServerAccounts(serverId: number): Promise<void> {
    const accounts = await db.select().from(sftpAccounts).where(eq(sftpAccounts.server_id, serverId));
