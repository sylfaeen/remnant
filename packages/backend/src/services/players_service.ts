...
class PlayersService extends EventEmitter {
  ...
  private serverPlayers: Map<number, Set<string>> = new Map();

  /**
   * Get current players for a server
   */
  getPlayers(serverId: number): Array<string> {
   ...
   */
  getPlayerCount(serverId: number): number {
    const players = this.serverPlayers.get(serverId);
   ...
   */
  parseLogLine(serverId: number, line: string): boolean {
   ...
   */
  clearPlayers(serverId: number): void {
    const players = this.serverPlayers.get(serverId);
   ...
   */
  private emitPlayersUpdate(serverId: number): void {
