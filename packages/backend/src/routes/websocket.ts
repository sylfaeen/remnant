...
const serverConnections = new Map<number, Set<WebSocket>>();

const metricsSubscriptions = new Map<number, Set<WebSocket>>();

const METRICS_POLL_INTERVAL = 5000;
const WS_MESSAGE_RATE_LIMIT = 30;
const WS_MESSAGE_RATE_WINDOW = 1000;

const metricsIntervals = new Map<number, NodeJS.Timeout>();

function startMetricsPolling(serverId: number) {
...
function stopMetricsPolling(serverId: number) {
...
function addMetricsSubscription(serverId: number, socket: WebSocket) {
...
function removeMetricsSubscription(serverId: number, socket: WebSocket) {
  const subscribers = metricsSubscriptions.get(serverId);
  if (subscribers) {
...
serverProcessManager.on('server:started', ({ serverId, pid }: { serverId: number; pid: number }) => {
...
serverProcessManager.on(
  'server:stopped',
  ({ serverId, code, signal }: { serverId: number; code: number | null; signal: string | null }) => {
...
serverProcessManager.on('server:error', ({ serverId, error }: { serverId: number; error: string }) => {
  const connections = serverConnections.get(serverId);
  if (!connections) return;
...
function addConnection(serverId: number, socket: WebSocket) {
...
function removeConnection(serverId: number, socket: WebSocket) {
  const connections = serverConnections.get(serverId);
  if (connections) {
...
export async function registerWebSocketRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/ws/console', { websocket: true }, async (socket: WebSocket, request) => {
    ...
    if (isNaN(serverId)) {
      socket.send(JSON.stringify({ type: 'error', message: 'Invalid serverId' }));
      socket.close(4005, 'Invalid serverId');
      return;
    }
