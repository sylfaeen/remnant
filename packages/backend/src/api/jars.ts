...
export const jarsRoutes = s.router(contract.jars, {
  getVersions: async ({ request }) => {
    try {
      ...
      const versions = await paperMCService.getVersions();
  ...
  getBuilds: async ({ request, query }) => {
    try {
      ...
      const builds = await paperMCService.getBuilds(query.version);
  ...
  download: async ({ request, params, body }) => {
    try {
      ...
      const result = await paperMCService.downloadJar(body.version, targetBuild, server.path, (progress) => {
