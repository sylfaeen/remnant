...
export const serversRoutes = s.router(contract.servers, {
  ...
  byId: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:control');
      const server = await serverService.getServerById(Number(params.id));
  ...
  listBackups: async ({ request, params }) => {
    try {
      ...
      const backups = result.backups.map((b) => ({
        filename: b.name,
        size: b.size,
        created: b.date,
      }));
  ...
  deleteBackup: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:control');
      checkRateLimit(`user:${user.sub}:servers.deleteBackup`, 5, ONE_MINUTE);
  ...
  backup: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:control');
      checkRateLimit(`user:${user.sub}:servers.backup`, 5, ONE_MINUTE);

      const id = Number(params.id);
      const result = await serverService.backupServer(id, body.paths);
