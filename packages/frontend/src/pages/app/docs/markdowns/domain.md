...
The configuration file is located at `/etc/nginx/sites-available/remnant`. After installation, it looks like this:
...
server {
    ...
    location /docs/ {
        alias $REMNANT_HOME/app/docs/.vitepress/dist/;  # Replace $REMNANT_HOME with your actual path
...
```bash
sudo nano /etc/nginx/sites-available/remnant
```
...
Once HTTPS is active, add this variable to your Remnant `.env` file:
...
```bash
sudo systemctl restart remnant
```
...
Caddy automatically provisions and renews the HTTPS certificate. Once active, add `SECURE_COOKIES=true` to your `.env` and restart Remnant.
...
[Security](/guide/security) - Learn more about Remnant's security features
