...
} from '@remnant/frontend/pages/app/features/sidebar_context';
...
export function Sidebar() {
  ...
  return (
    <>
      {mobileOpen && (
        <div
          className={'fixed inset-0 z-20 bg-black/40 transition-opacity min-[960px]:hidden'}
...
function SidebarSection({ section, location }: SidebarSectionProps) {
  return (
    <div>
      {section.section && (
        <div
...
function SidebarItem({ item, location }: SidebarItemProps) {
  const { t } = useTranslation();
...
type SidebarChildrenProps = {
  items: Array<SidebarNavChild>;
  location: { pathname: string };
};

function SidebarChildren({ items, location }: SidebarChildrenProps) {
  const { t } = useTranslation();
