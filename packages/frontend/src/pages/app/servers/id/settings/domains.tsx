...
function PlayerDomainSection({ serverId }: PlayerDomainSectionProps) {
  ...
  const tcpDomains = domainsData?.domains?.filter((d) => d.type === 'tcp') ?? [];
  ...
  return (
    <FeatureCard>
      <FeatureCard.Header>
        <FeatureCard.Content>
          <FeatureCard.Title count={tcpDomains.length > 0 && tcpDomains.length}>
        ...
        {tcpDomains.length === 0 ? (
          ...
          <>
            {tcpDomains.map((domain) => (
              <TcpDomainRow key={domain.id} domain={domain} onRemove={removeDomain} {...{ serverIp }} />
            ))}
            {showForm && (
...
type TcpDomainRowProps = {
  domain: TcpDomainData;
  serverIp: string;
  onRemove: { mutateAsync: (id: number) => Promise<unknown>; isPending: boolean };
};

function TcpDomainRow({ domain, serverIp, onRemove }: TcpDomainRowProps) {
  const { t } = useTranslation();
