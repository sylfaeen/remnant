...
export function Textarea({ className, variant, error, ref, ...props }: TextareaProps) {
  return (
    <textarea
      className={cn(
        'duration-(--duration-fast focus:ring-offset- flex min-h-20 w-full resize-y py-3 text-zinc-900 transition-all placeholder:text-zinc-600 focus:ring-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-4',
        variant === 'ghost' &&
        ...
        (variant === 'default' || !variant) &&
          'border border-black/10 bg-white hover:border-black/12 focus:border-zinc-900 focus:ring-zinc-900/20',
