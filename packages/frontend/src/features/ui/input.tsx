...
export const inputVariants = cva(
  [
    'flex w-full text-zinc-900 placeholder:text-zinc-600 transition-all duration-(--duration-fast) disabled:cursor-not-allowed disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-offset-0',
  ],
  {
    variants: {
      variant: {
...
export function InputGroup({ label, error, hint, required, children, className }: InputGroupProps) {
  return (
    <div className={cn('flex flex-col gap-1.5', className)}>
      ...
      {hint && !error && <p className={'text-sm text-zinc-600'}>{hint}</p>}
    </div>
  );
}
