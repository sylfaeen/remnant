  ...
  return (
    <div className={'relative'} ref={dropdownRef}>
      <button
        ...
        className={cn(
          'flex size-9 items-center justify-center rounded-lg text-zinc-500 transition-all duration-(--duration-fast) hover:bg-zinc-100 hover:text-zinc-700',
          isOpen && 'bg-zinc-100 text-zinc-700'
        )}
      ...
      {isOpen && (
        <div
          className={
            'animate-scale-in absolute top-full right-0 mt-2 w-36 origin-top-right rounded-lg border border-zinc-200 bg-white py-1 shadow-lg'
          }
        >
          {languages.map((lang) => (
            <button
...
export function LanguageSelectorCompact() {
  ...
  return (
    <button
      ...
      className={
        'flex size-9 items-center justify-center rounded-lg text-zinc-600 transition-all duration-(--duration-fast) hover:bg-zinc-100 hover:text-zinc-900'
      }
