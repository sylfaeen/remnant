...
validate_type() {
  local type="$1"
  if ! [[ "$type" =~ ^(http|tcp|panel)$ ]]; then
    json_error "Invalid type: must be http, tcp, or panel"
  fi
}
...
action_add() {
  ...
  if [ "$type" = "tcp" ]; then
    # TCP domains are DNS-only (A record + SRV record)
...
action_list() {
  ...
  for vhost in "${SITES_AVAILABLE}"/*; do
    ...
    # Skip the default remnant vhost
    if [ "$name" = "remnant" ] || [ "$name" = "default" ]; then
      continue
    fi
