...
restart_services() {
    ...
    if grep -q "ReadWritePaths=" /etc/systemd/system/remnant.service 2>/dev/null; then
        sed -i 's|ReadWritePaths=.*|ReadWritePaths=/opt/remnant /etc/nginx /etc/letsencrypt /var/lib/letsencrypt /var/lib/nginx /var/log/nginx /var/log/letsencrypt /run|' /etc/systemd/system/remnant.service
