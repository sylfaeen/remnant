...
show_status() {
    local status_script="${REMNANT_HOME}/app/scripts/cli/show_status.sh"
...
do_update() {
    local update_script="${REMNANT_HOME}/app/scripts/cli/do_update.sh"
...
show_domains() {
    local domains_script="${REMNANT_HOME}/app/scripts/cli/show_domains.sh"
    if [[ ! -f "$domains_script" ]]; then
        echo -e "  ${RED}✗${NC} Domains script not found at ${domains_script}"
        exit 1
    fi
    sudo bash "$domains_script"
}

do_uninstall() {
    local uninstall_script="${REMNANT_HOME}/app/scripts/cli/do_uninstall.sh"
    if [[ ! -f "$uninstall_script" ]]; then
        echo -e "  ${RED}✗${NC} Uninstall script not found at ${uninstall_script}"
        echo -e "  ${GRAY}Run manually:${NC} ${CYAN}curl -fsSL https://raw.githubusercontent.com/sylfaeen/remnant/main/scripts/uninstall.sh | sudo bash${NC}"
        exit 1
    fi
