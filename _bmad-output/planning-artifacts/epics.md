...
- FR52: Router ts-rest pour exposer les opérations firewall au frontend
...
| FR52 | Epic 12 | Router ts-rest firewall |
...
- Fichiers cles:
  - Nouveau: `packages/backend/src/routes/handlers/settings.ts` — router `settings`
  - `packages/backend/src/routes/index.ts` — enregistrement du router
...
- Fichiers cles:
  ...
  - Nouveau: `packages/backend/src/routes/handlers/java.ts`
  - `packages/backend/src/routes/index.ts` — enregistrement du router
...
### Story 12.3: Backend — Router ts-rest Firewall

#### Story

**As a** développeur,
**I want** un router ts-rest pour exposer les opérations firewall,
...
##### AC1: Router firewall
**Given** le router ts-rest est enregistré
...
- Endpoint ts-rest `users.updateLocale`
...
- Gestion d'erreur `PORT_ALREADY_IN_USE` dans le router ts-rest (code CONFLICT)
